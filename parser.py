...
class NYKParser:
    ...
    """Takes latest feed items and checks whether they are new"""
        ...
        while feed.items[i].title != self.latest_title:
    ...
    def save(self):
    """Recodrds the latest title to the database"""
    
    c = self.db.cursor()
    c.execute('DELETE FROM titles')
    c.execute('INSERT INTO titles VALUES (?)', self.new_titles[-1][0])
    
    self.db.info('db altering success, exiting')
    

NYKParser = NYKParser()

NYKParser.connect()
NYKParser.get_latest_titles()
NYKParser.parse()
NYKParser.send()
NYKParser.save()
